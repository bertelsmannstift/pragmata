...
def upsert_record_metadata(
    ...
    *,
    ...
    """Merge ``updates`` into ``record.metadata`` and upsert the FULL dict.

    Responses, suggestions, vectors, and other record state are preserved
    because the payload omits them (Argilla server v2.8.0 preserves absent
    fields on upsert). Only ``id`` and the full merged metadata are sent.

    ``remove_keys`` are dropped from the merged dict before upsert, allowing
    idempotent clear of advisory tags (e.g. ``needs_completion``).
    """
    merged = dict(record.metadata)
    merged.update(updates)
    for key in remove_keys:
        merged.pop(key, None)
    if merged == dict(record.metadata):
        return  # no-op
    payload = {"id": record.id, "metadata": merged}
    dataset.records.log([payload])
