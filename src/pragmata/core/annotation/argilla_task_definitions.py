...
from pragmata.core.annotation.locales import CATALOGS, get_catalog
...
def _discard_i18n_payload_for_locale(loc: Locale, task: Task) -> dict[str, Any]:
    ...
    return {
        "panel_summary": catalog[(task, "widget", "discard.panel_summary")],
        "panel_help": catalog[(task, "widget", "discard.panel_help")],
        "reason_label": catalog[(task, "widget", "discard.reason_label")],
        "reason_placeholder": catalog[(task, "widget", "discard.reason_placeholder")],
        "notes_label": catalog[(task, "widget", "discard.notes_label")],
        "notes_placeholder": catalog[(task, "widget", "discard.notes_placeholder")],
        "button_label": catalog[(task, "widget", "discard.button_label")],
        "discard_reason_title": catalog[(task, "question", "discard_reason")],
        "discard_notes_title": catalog[(task, "question", "discard_notes")],
        "reason_options": [
...
def _render_discard_template(template_text: str, task: Task, dataset_locale: Locale) -> str:
    ...
    Uses literal ``__MARKER__`` placeholders rather than ``string.Template``
    because the JS body contains ``$nuxt`` / ``$i18n`` references that would
    otherwise collide with Template's ``$`` substitution syntax.
    """
    ...
    rendered = rendered.replace("__I18N_JSON__", json.dumps(i18n_payload, ensure_ascii=False))
    rendered = rendered.replace("__SUPPORTED_LOCALES_JSON__", json.dumps([loc.value for loc in locales_in_order]))
    rendered = rendered.replace("__DEFAULT_LOCALE_JSON__", json.dumps(dataset_locale.value))
    return rendered
