...
Add `import_eval_score_frame` to `core/eval/imports.py`, parallel to the existing train and predict import functions. It is the score ingestion boundary: it reads the resolved score input CSV, prepares it for the task-specific score contract, and applies `validate_eval_score_frame`. Direct labeled inputs and annotation exports are already Pragmata-shaped and pass through without text-column normalization. Multiple annotator rows for the same scoring unit are then consolidated to one row by per-label majority (`consolidate_labels_by_majority`, shared with train ingestion; a no-op when units are already unique); a uniqueness guard runs afterwards as a post-collapse invariant and hard-errors only on a residual duplicate the collapse cannot resolve (e.g. two `chunk_id`s sharing a `chunk_rank`). Annotator disagreement is folded into a single majority label here and is out of scope for the score report's CIs (it remains the IAA report's concern).
...
2. **Incomplete and degenerate scoring data.** *(Partially resolved.)* Multiple annotator rows for the same unit are now consolidated by per-label majority at ingestion, reusing the same `consolidate_labels_by_majority` helper as `eval train`. Still open: retrieval metrics assume complete ranked chunk labels per query, so unlabeled chunks need a deliberate policy (fail, skip affected queries, or a caveated fallback), and all-0/all-1 or otherwise degenerate labels can make some estimates uninformative and should be handled explicitly.
